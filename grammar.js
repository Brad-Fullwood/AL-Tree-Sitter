...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    [$.name_or_keyword, $._atom], // Resolve conflict for key_declaration parsing
    [$.name, $._atom], // Resolve name vs atom conflict in key_declaration
  ...
  // We use placeholders that the generator fills based on TM scopes.
  ...
  _object_body_item: $ => choice(
    ...
    $.object_var_section,
    $.empty_var_section,
    // Be permissive: allow declarations to continue after a var section.
    ...
    $.parenthesized_block,
    $.bracketed_block,
    $.semicolon, // Allow orphan semicolons
  ),
  ...
  // Key declarations in table keys section: key(Name; field1, field2) { ... }
  key_declaration: $ => prec.right(seq(
    field('keyword', $.keyword), // 'key'
    '(',
  ...
  case_branch: $ => choice(
    ...
    seq(
      field('labels', $.case_label_list),
      field('sep', ':'),
      $.semicolon, // Empty branch like "Label:;"
    ),
  ),
  ...
  // Case labels should not start with unary +/-; keep them simple to avoid ambiguity.
